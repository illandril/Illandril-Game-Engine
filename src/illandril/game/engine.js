...
illandril.game.Engine.init = function( gameContainerID ) {
  ...
  if ( illandril.game.Engine.map != null ) {
    ...
    for ( var idx = 0; idx < illandril.game.Engine.map.length; idx++ ) {
      ...
      new illandril.game.objects.GameObject( world, illandril.math.Bounds.fromCenter( new goog.math.Vec2( objDef.x, objDef.y ), new goog.math.Vec2( objDef.width, objDef.height ) ), objDef.bg );
    }
    world.endBulk();
  }
  
  // Start for testing
  charac = new illandril.game.objects.Player( world, illandril.math.Bounds.fromCenter( new goog.math.Vec2( 250, 100 ), new goog.math.Vec2( 8, 8 ) ) );
  ...
  for ( var i = lowMob; i < highMob; i++ ) {
    ...
    if ( bounds != null ) {
      mobs[i] = new illandril.game.objects.Collectable( world, bounds );
